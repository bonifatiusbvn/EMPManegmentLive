...
namespace EMPManegment.Web.Controllers
{
    ...
    public class ExpenseMasterController : Controller
    {
        ...
        [HttpPost]
        public async Task<IActionResult> GetExpenseDetailsList([FromBody] AGGridRequestModel ExpenseRequest)
        {
            try
            {
                ExpenseRequest.filters ??= new List<FilterModel>();

                var ExpenseDetails = await APIServices.AGPostAsync<ExpenseDetailsView>(ExpenseRequest, "ExpenseMaster/GetExpenseDetailList");

                return new JsonResult(new
                {
                    rowsThisPage = ExpenseDetails.Data,
                    totalRowCount = ExpenseDetails.RecordsTotal
                });
            }
            catch (Exception ex)
            {
        ...
        [HttpPost]
        public async Task<IActionResult> GetUserListTable([FromBody] AGGridRequestModel ExpenseRequest)
        {
            try
            {
                ExpenseRequest.filters ??= new List<FilterModel>();

                var ExpenseDetails = await APIServices.AGPostAsync<UserExpenseDetailsView>(ExpenseRequest, "ExpenseMaster/GetUserList");
        ...
        [HttpPost]
        public async Task<IActionResult> GetUserExpenseList([FromBody] AGGridRequestModel ExpenseRequest)
        {
            try
            {
                ExpenseRequest.filters ??= new List<FilterModel>();

                var ExpenseDetails = await APIServices.AGPostAsync<ExpenseDetailsView>(ExpenseRequest, "ExpenseMaster/GetUserExpenseDetail");
